...
int main(int argc,char *argv[]) {
  ...
  if(!(fp0 = fopen("visu/disk_mesh_1.vtk","r"))){
    printf("%s\n", "input file open error.");
    return -1;
  }
  //grid_number = ReadMeshPoints(sx, sy, sz);

  sprintf(fname,"%s/Drawpoint/%s%s_t_%.2f_R_%.2f_%s_betacor.vtk",DIR,func_type,boundary,landau_t,Rad,argv[1]);
  fp1 = fopen(fname,"w");
